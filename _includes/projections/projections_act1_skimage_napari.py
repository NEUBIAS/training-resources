...
import re
img = BioImage("https://github.com/NEUBIAS/training-resources/raw/master/image_data/xyz_16bit__spots.tif")
print(img.shape)
print(img.dims)
print(img.dims.order)
scales = list(img.physical_pixel_sizes)
print(scales)
match = re.search(r'unit=(.+)', img.metadata)
...
viewer.add_image(img_data, scale=scales)
...
max_per_slice = np.iinfo(img_data.dtype).max
...
max_x_image = np.max(img_data, axis=2)
max_y_image = np.max(img_data, axis=1)
